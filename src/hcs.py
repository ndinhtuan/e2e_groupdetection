...
def highly_connected(G, E, highly_connected_rate=0.3):
    ...
    return len(E) > len(G.nodes) * highly_connected_rate
...
def HCS(G, highly_connected_rate=0.3):
    """Basic HCS Algorithm
    ...
    if not highly_connected(G, E, highly_connected_rate=highly_connected_rate):
        G = remove_edges(G, E)
        sub_graphs = [G.subgraph(c).copy() for c in nx.connected_components(G)]

        if len(sub_graphs) == 2:
            H = HCS(sub_graphs[0], highly_connected_rate=highly_connected_rate)
            _H = HCS(sub_graphs[1], highly_connected_rate=highly_connected_rate)
...
def labelled_HCS(G, highly_connected_rate=0.3):
    """
    ...
    _G = HCS(G, highly_connected_rate=highly_connected_rate)
