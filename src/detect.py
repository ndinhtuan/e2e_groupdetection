...
import torch
from tracking_utils.utils import mkdir_if_missing, xyxy2tlwh
...
def eval_seq(opt, dataloader, data_type, save_dir=None, show_image=True, frame_rate=30):
    ...
    for path, img, img0 in dataloader:
        ...
        result_filename = os.path.join(save_dir, "{}.txt".format(name_file))
        ...
        timer.toc()
        # save results
        results.append((class_id, tlwhs, scores))
        # save results
...
def main(opt, data_root='/data/tuannd/fformation/data/group_detection', det_root=None, seqs=('sample_data',), exp_name='demo', save_images=False, save_videos=False, show_image=True):

    logger.setLevel(logging.INFO)
    result_root = os.path.join(data_root, '..', 'dets', exp_name)
    print(result_root)
    mkdir_if_missing(result_root)
    data_type = 'group_detection'
    ...
    for seq in seqs:
        ...
        nf, ta, tc = eval_seq(opt, dataloader, data_type,
...
if __name__ == '__main__':
    ...
    opt = opts().init()
    if opt.val_hie:
        seqs_str = '''sample_data'''
        data_root = '/data/tuannd/fformation/data/group_detection'
    elif opt.test_hie:
        seqs_str = '''sample_data'''
        data_root = '/data/tuannd/fformation/data/group_detection'
    else:
